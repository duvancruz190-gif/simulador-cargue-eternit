...
if not st.session_state.autenticado:
    ...
    .saldo-box { background:#b7950b; color:white; text-align:center; padding:8px; margin:4px; border-radius:5px; font-size:10px; font-weight:800; border:1px solid #7d6608; min-height: 50px; }
    </style>
    ...
    if pedido_items:
        ...
        for item in pedido_sorted:
            ...
            # Consolidación de saldos (Máximo 60 por bloque amarillo)
            while sobra > 0:
        ...
        todo_cargado_verdes = True

        # RENDER DE FILAS DOBLES (Paquetes Verdes + Saldos a los lados)
        for row in rows:
            ...
            if largo_ocupado + avance <= limite_ft:
                ...
                todo_cargado_verdes = False

        # RENDER DE PAQUETE IMPAR (Atravesado)
        if atravesado:
        ...
        # RENDER DE SALDOS SOBRANTES (Si queda largo, llenar laterales aunque no haya paquetes verdes)
        while saldos_render and (largo_ocupado + 4 <= limite_ft):
            ...
            with cols_s[3]:
                if saldos_render:
                    s = saldos_render.pop(0)
                    st.markdown(f'<div class="saldo-box">{s["label"]}<br>{s["cant"]} UND</div>', unsafe_allow_html=True)
            largo_ocupado += 4 # Espacio estimado para una fila de saldos

        # --- ALERTAS FINALES ---
        if not todo_cargado_verdes:
            st.error("⚠️ NO CABEN TODOS LOS PAQUETES COMPLETOS POR LARGO DEL VEHÍCULO.")
        
        if saldos_render:
            st.warning(f"⚠️ NO HUBO ESPACIO PARA {len(saldos_render)} SALDO(S) EN LOS LATERALES.")
